...
//! ```
//!
// terax vendoring: this crate is pulled in via [patch.crates-io] as a local
// path dependency (to drop PSUEDOCONSOLE_INHERIT_CURSOR). As a path dep its
// lints are not capped, so the workspace's `cargo clippy -- -D warnings` would
// otherwise escalate upstream warnings into build errors. Silence them — we do
// not own this third-party code.
