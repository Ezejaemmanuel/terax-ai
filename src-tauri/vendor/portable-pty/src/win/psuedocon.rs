    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // PSUEDOCONSOLE_INHERIT_CURSOR intentionally omitted (terax patch):
                // on Windows < 11 24H2 it makes ClosePseudoConsole deadlock on
                // teardown (microsoft/terminal#17688), which froze new terminals
                // while the lifecycle lock was held. A GUI terminal spawning a
                // fresh shell has no parent console cursor to inherit anyway.
